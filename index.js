...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.bbvd7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        app.post('/tasks', async (req, res) => {
            ...
            const product = await taskCollection.insertOne(cursor);
        ...
        app.post('/submit', async (req, res) => {
            const cursor = req.body;
            const product = await submittedTaskCollection.insertOne(cursor);
            console.log('hit the post api' , cursor);
            res.json(product);
        })
